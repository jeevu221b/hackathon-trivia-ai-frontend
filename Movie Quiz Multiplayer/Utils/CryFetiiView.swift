...
struct CryConfettiModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        if #available(iOS 17.0, *) {
            content
                .overlay(isActive ? CryFettiView().opacity(opacity) : nil)
...
extension View {
    func displayCryfetti(isActive: Binding<Bool>) -> some View {
        self.modifier(CryConfettiModifier(isActive: isActive))
    }
}
